...
public class TradeAuthFiRe
{
	...
	public  byte[] bIn_RRN=new byte[0];          //[37]检索参考号[从原交易的'38'获取]
	...
	public PKGResult seal()
	{
		...
		byte[] bBatNo2= (new String("21"+(new String(bIn_BatNo)+"000"))).getBytes();
		...
		trade_rq.setField(22, (new String("012")).getBytes());       //022:服务点输入方式码,固定值'072'
	...
	public PKGResult unseal(byte pbPkg[])
	{
		...
		//1:解63域的国际信用卡公司代码
		byte b[] = trade_rs.getField(63);
		if( b != null )
		{
			System.arraycopy(b,0,bOut_CCCCode,0,3);
		}
